#code to generate Video1 of the Jasek et al desmosomal connectome paper
#Gaspar Jekely 

source("code/Packages_and_Connection.R")

# read cells and meshes --------------------------------------------------------------
{
...
# define cell type list and read cells ---------------------------------------------------
...
for (j in c(37:89)){    #iterate through the celltype list 
  ...
  nview3d("ventral", extramat=rotationMatrix(pi/20, 0, 1, 1))
  par3d(zoom=0.5)
  ...
  name1 <- catmaid_get_neuronnames(unlist(celltypelist[[j]][1])[1], pid=11)
  name1 <- sub("_.*$", "", name1)
  print(name1)
  #add a text label
...
texts3d(115000,30000, 8020, text = "all muscles", col='black', cex = 1.5)
...
for (i in c(6:-6)){
  clear3d(type = "lights")
  next3d(clear=F)
  rgl.light(i*10, 30, diffuse = "gray70")
  rgl.light(i*10, 30, specular = "gray5")
  rgl.light(-i*10, -30, specular = "gray5")
  next3d(clear=F)
  rgl.light(i*10, 30, diffuse = "gray70")
  rgl.light(i*10, 30, specular = "gray5")
  rgl.light(-i*10, -30, specular = "gray5")
  rgl.snapshot(paste("videos/Video1_MUScelltypes_light", -i+23, ".png", sep = ""))
}
